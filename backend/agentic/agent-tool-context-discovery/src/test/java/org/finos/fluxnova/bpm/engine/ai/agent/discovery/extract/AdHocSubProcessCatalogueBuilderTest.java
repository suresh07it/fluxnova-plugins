...
class AdHocSubProcessCatalogueBuilderTest {
    ...
    @Test
    void build_fullCreditCheckScenario() {
        String bpmn = """
                <?xml version="1.0" encoding="UTF-8"?>
                <definitions xmlns="http://www.omg.org/spec/BPMN/20100524/MODEL"
                             xmlns:camunda="http://camunda.org/schema/1.0/bpmn">
                  <process id="p">
                    <adHocSubProcess id="creditCheckAgent">
                      ...
                      <serviceTask id="riskAssessment" name="Risk Assessment">
                        <documentation>Assesses risk based on credit score and application amount.</documentation>
                        <extensionElements>
                          <camunda:inputOutput>
        ...
        AgentToolEntry credit = catalogue.findById("creditScoreCheck").orElseThrow();
        ...
        AgentToolEntry risk = catalogue.findById("riskAssessment").orElseThrow();
        assertEquals("Risk Assessment", risk.name());
